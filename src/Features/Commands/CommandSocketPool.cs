...
{
  private readonly CommandClient[] _sockets;
    ...
    public CommandSocketPool(CommandClient[] sockets)
    {      
        _sockets = sockets;
    ...
    /// </summary>
    public CommandClient GetSocketByHash(int hash)
    {
        if (_count == 1)
            return _sockets[0];
    ...
    /// </summary>
    public CommandClient GetLeastLoadedSocket()
    {
        if (_count == 1)
            return _sockets[0];
    ...
    /// </summary>
    public void ReleaseSocket(CommandClient socket)
    {
        if (_count == 1) return;
    ...
    /// </summary>
    public IReadOnlyList<CommandClient> Sockets => _sockets;
